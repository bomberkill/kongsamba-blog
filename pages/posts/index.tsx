...
export default function Posts({
  ...
  const article = articles.data
    ? articles.data.getAllArticles.filter((item) => item.articleInput.type === 'article')
    : [];
  const breve = articles.data
    ? articles.data.getAllArticles.filter((item) => item.articleInput.type === 'breve')
    : [];
  const cover = articles.data
    ? articles.data.getAllArticles.filter((item) => item.articleInput.type === 'cover')
    : [];
  const news = articles.data
    ? articles.data.getAllArticles.filter((item) => item.articleInput.type === 'news')
    : [];
  const sport = articles.data
    ? articles.data.getAllArticles.filter((item) => item.articleInput.type === 'sports')
    : [];
  useEffect(() => {
    setTimeout(() => {
      setDelayOver(false);
                    ...
                    article.map((item, index) => (
                      ...
                      >
                        ...
                        <Stack justify="space-between" mt="md" mb="xs">
                          <Text fw={500}>{item.articleInput.title}</Text>
                          <Badge
                            radius={5}
                            variant="light"
                            gradient={{ from: 'pink', to: 'yellow' }}
                          >
                            {item.articleInput.type}
                          </Badge>
                        </Stack>

                        <Text lineClamp={3} size="sm" c="dimmed">
                          {item.articleInput.message}
                        </Text>
